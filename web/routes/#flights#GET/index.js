...
export async function fulfill(httpRequest, context, pathParams) {
  ...
  let message = {};
  if (httpRequest.request_values?.query_string_values?.messageId) {
    message = framework.cache.get(`message:${httpRequest.request_values.query_string_values.messageId}`);
    message = framework.helpers.jsonHelper.parse(message);
    console.log(message);
  }

  let html = fs.readFileSync(path.join(thisDirName, 'index.html')).toString();
  html = nunjucks.renderString(html, {
    createFlightUrl: '/create-flight',
    deleteUrl: '/api/flights',
    footerHtml,
    headerHtml,
    jsCode,
    ...
    rowData: JSON.stringify(flights)
  });
