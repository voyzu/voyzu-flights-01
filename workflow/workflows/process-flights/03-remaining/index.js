 ...
 * @param {*} keyName Name of the redis key that holds the workflow job.
 */
export async function fulfill(channelName, keyName) {
    ...
    // This is the last step in the workflow, so there is no next step

    // We now delete the key
    await redisClient.DEL(keyName);

    log.debug(`processed key ${keyName}`);
